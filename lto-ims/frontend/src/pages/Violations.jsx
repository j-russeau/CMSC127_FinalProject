...
const demoTickets = [
  {
    ticket_id: "VIO-2024-001", driver_name: "Juan dela Cruz",
    driver_license: "N01-12-3456789", plate_number: "ABC-1234",
    vehicle_label: "Toyota Vios 2023", datetime: "2026-04-25 14:30:00",
    issued_at: "Makati City, NCR", total_fine: 7500, violation_status: "unpaid",
  },
  {
    ticket_id: "VIO-2024-002", driver_name: "Maria Santos",
    driver_license: "N02-13-4567890", plate_number: "XYZ-5678",
    vehicle_label: "Honda City 2022", datetime: "2026-04-24 10:15:00",
    issued_at: "Quezon City, NCR", total_fine: 1500, violation_status: "paid",
  },
  {
    ticket_id: "VIO-2024-003", driver_name: "Pedro Garcia",
    driver_license: "N03-14-5678901", plate_number: "DEF-9012",
    vehicle_label: "Yamaha Mio 2024", datetime: "2026-04-23 16:45:00",
    issued_at: "Manila, NCR", total_fine: 500, violation_status: "contested",
  },
];
...
const emptyTicketForm = {
  ticket_id: "", datetime_local: "", issued_at: "", apprehending_officer: "",
...
export default function Violations() {
   ...
   * Used in the create modal to auto-fill engine/chassis when a plate is entered,
   * since officers won't memorise all three identifiers.
   */
  async function loadVehicleMap() {
    try {
      const rows = await listVehicles();
      const map  = {};
      (rows || []).forEach((v) => { map[v.plate_number] = v; });
      setVehicleMap(map);
    } catch {
      // non-critical — plate lookup just won't auto-fill
    }
  }
  ...
  /*
   * When the plate field changes in the create modal, look it up in the pre-loaded
   * vehicle map and auto-fill engine and chassis numbers. Makes the form usable
   * without memorising three separate identifiers.
   */
  useEffect(() => {
  ...
  async function submitCreateTicket() {
    ...
    for (const [label, value] of required) {
      if (!value || String(value).trim() === "") { setModalErr(`${label} is required.`); return; }
    }
    ...
    if (cleaned.length === 0) { setModalErr("Add at least one violation."); return; }

    for (let i = 0; i < cleaned.length; i++) {
      const v = cleaned[i];
      if (!v.name) { setModalErr(`Violation ${i + 1}: name is required.`); return; }
      const fine = Number(v.fine);
      if (!Number.isFinite(fine) || fine < 0) { setModalErr(`Violation ${i + 1}: fine must be ≥ 0.`); return; }
    }
          ...
          {filtered.map((t) => {
            ...
            return (
              <div className={`tableRow${isSelected ? " selectedRow" : ""}`} key={t.ticket_id}>
                ...
                <div className="cell"><StatusPill status={t.violation_status} /></div>
                <div className="cell">
        ...
        {selectedTicketId && (
          <div className="card">
            <div className="detailsHeader">
              ...
              <button className="linkBtn muted" onClick={() => { setSelectedTicketId(null); setViolations([]); }}>
                Close
              </button>
            </div>
            ...
            {selectedTicket && (
              <>
                <div className="detailsMeta">
                  <div><span className="metaLabel">Driver:</span> {selectedTicket.driver_license}</div>
                  <div><span className="metaLabel">Vehicle:</span> {selectedTicket.plate_number}</div>
                  <div><span className="metaLabel">Location:</span> {selectedTicket.issued_at}</div>
                  <div>
                ...
                {/* Status change buttons — only show the statuses the ticket isn't already in */}
                <div className="statusUpdateRow">
                  {["paid", "unpaid", "contested"]
                    .filter((s) => s !== selectedTicket.violation_status)
                    .map((s) => (
                      <button key={s} className={`statusUpdateBtn statusUpdateBtn--${s}`}
            ...
            <div className="detailsBody">
              ...
              {loadingViolations ? (
                ...
                <div className="softNote">No violations on record.</div>
              ) : (
          ...
          <div className="modalOverlay" onClick={() => setCreateOpen(false)}>
            <div className="modal wide" onClick={(e) => e.stopPropagation()}>
              <div className="modalHeader">
                ...
                <button className="modalClose" onClick={() => setCreateOpen(false)}>✕</button>
              </div>
              ...
              <div className="stepBox stepBlue">
                ...
                <div className="formGrid2">
                  <div className="field">
                    ...
                    <input className="input" placeholder="VIO-20240101-XXXX"
                  ...
                  <div className="field">
                    ...
                    <input className="input" placeholder="Makati City, NCR"
                  ...
                  <div className="field">
                    ...
                    <input className="input" placeholder="Officer Name"
                  ...
                  {/* Plate auto-fills engine + chassis from the pre-loaded vehicle map */}
                  <div className="field">
                  ...
                  <div className="field">
                    ...
                    <input className="input" placeholder="Auto-filled from plate"
              ...
              <div className="stepBox stepRed">
                <div className="stepRow">
                  ...
                  <button className="addBtn" onClick={addViolationRow}>+ Add Violation</button>
                </div>
                    ...
                    <div className="violationRow" key={idx}>
                      <div className="violationRowTop">
                        ...
                        {modalViolations.length > 1 && (
                          <button className="removeBtn" onClick={() => removeViolationRow(idx)}>Remove</button>
                        )}
                      </div>
                      <div className="violationInputs">
                        <input className="input" placeholder="Violation Name (e.g. Reckless Driving)"
                          value={v.name}
                          onChange={(e) => updateViolationRow(idx, { name: e.target.value })} />
                        <input className="input" type="number" min="0" step="0.01" placeholder="Fine Amount"
                          value={v.fine}
              ...
              <div className="modalFooter">
                <button className="secondaryBtn" onClick={() => setCreateOpen(false)}>Cancel</button>
